  ...
  useEffect(() => {
    api
      .get<PaginatedList<MaintenanceReport> | MaintenanceReport[]>(
        `/v1/reports${buildQuery({ pageSize: 50 })}`,
      )
      .then((result) => {
    ...
    <>
      <div className="page-header">
        <div className="page-header-left">
          <h2>Requests</h2>
          <p>Review incoming maintenance work before assignment.</p>
        </div>
      </div>
            ...
            {loading ? (
              ...
              requests.map((request) => (
                <tr key={request.id}>
                  ...
                  <td>
                    <span
                      className={`badge badge-${request.priority ?? "low"}`}
                    >
                      {request.priority ?? "unassigned"}
                    </span>
                  </td>
                  <td>
                  ...
                  <td className="table-muted">{timeAgo(request.createdAt)}</td>
                  <td>
                    <Link
