          ...
          {values.length ? (
            values.slice(0, 8).map((item) => (
              <div className="hbar-row" key={item.label}>
                <div className="hbar-label" title={item.label}>
                ...
                <div className="hbar-val">{item.value}</div>
              </div>
            ))
          ) : (
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="page-subtitle">
            Operational intelligence across campus maintenance.
          </div>
        </div>
      ...
      <div className="stat-cards">
        <div className="stat-card green">
          ...
          <strong className="stat-card-value">{stats?.total ?? 0}</strong>
        </div>
        <div className="stat-card red">
          <span className="stat-card-label">Overdue</span>
          <strong className="stat-card-value">{stats?.overdue ?? 0}</strong>
        </div>
        <div className="stat-card amber">
          <span className="stat-card-label">Reopened</span>
          <strong className="stat-card-value">{stats?.reopened ?? 0}</strong>
        </div>
      ...
      <div className="analytics-grid">
        ...
        <HorizontalBars
          ...
          values={report?.byBuilding ?? []}
        />
        <HorizontalBars
          title="Reports by department"
          values={report?.byDepartment ?? []}
        />
        <HorizontalBars
          title="Technician workload"
          values={report?.byTechnician ?? []}
        />
      </div>
    </div>
  );
