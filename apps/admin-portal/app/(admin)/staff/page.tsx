...
export default function StaffPage() {
  ...
  const saveEdit = async () => {
    ...
    setSaving(true);
    try {
      const updated = await fetchApi<UserProfile>(`/v1/admin/users/${editUser.id}`, {
        method: "PATCH",
        body: JSON.stringify({
          role: editRole,
  ...
  const deactivate = async (user: UserProfile) => {
    ...
    try {
      const updated = await fetchApi<UserProfile>(`/v1/admin/users/${user.id}`, {
        method: "PATCH",
  ...
  const filtered = staff.filter((u) => {
    ...
    if (searchFilter && !u.fullName.toLowerCase().includes(searchFilter.toLowerCase()) && !u.email.toLowerCase().includes(searchFilter.toLowerCase())) return false;
    return true;
  });
    ...
    <div>
      ...
      <div className="card">
        <div className="table-wrapper">
          {loading ? (
            <div style={{ padding: "48px", textAlign: "center", color: "var(--muted)" }}>Loading staff…</div>
          ) : filtered.length === 0 ? (
              ...
              <tbody>
                {filtered.map((u) => (
                  <tr key={u.id}>
                    ...
                    <td>
                      <span className={`badge ${u.role === "administrator" ? "badge-danger" : u.role === "supervisor" ? "badge-info" : "badge-gray"}`}>
                        {u.role}
                      </span>
                    </td>
                    <td className="text-muted text-sm">{u.departmentName ?? "—"}</td>
                    <td>
          ...
          <div className="modal modal-sm" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="alert alert-info mb-3">
                Email invitations are not yet available. Staff members must be provisioned through Supabase auth.
              </div>
              <div className="form-group">
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" disabled>Send Invite (Coming Soon)</button>
            </div>
          </div>
        </div>
      )}
